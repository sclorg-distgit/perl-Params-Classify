...
Name:           %{?scl_prefix}perl-Params-Classify
Version:        0.015
Release:        9%{?dist}
...
BuildRequires:  %{?scl_prefix}perl(ExtUtils::ParseXS) >= 3.30
BuildRequires:  %{?scl_prefix}perl(Module::Build)
# Module Runtime
BuildRequires:  %{?scl_prefix}perl(Devel::CallChecker) >= 0.003
BuildRequires:  %{?scl_prefix}perl(Exporter)
BuildRequires:  %{?scl_prefix}perl(parent)
BuildRequires:  %{?scl_prefix}perl(Scalar::Util) >= 1.01
...
Requires:       %{?scl_prefix}perl(Devel::CallChecker) >= 0.003
Requires:       %{?scl_prefix}perl(Exporter)
Requires:       %{?scl_prefix}perl(Scalar::Util) >= 1.01
...
%build
%{?scl:scl enable %{scl} '}perl Build.PL --installdirs=vendor --optimize="%{optflags}" && ./Build%{?scl:'}
...
%changelog
* Mon Jan 06 2020 Jitka Plesnikova <jplesnik@redhat.com> - 0.015-9
- SCL
